...
export default function PAGES_Verify() {
  ...
  const { verifyOtp: verifyOTP, signInWithPhone: sendOTP } = useAuthStore();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // BUG FIX: check store for profile immediately after verify.
      // useAuthStore.getState() reads the updated state synchronously
      // after verifyOtp sets the user. onAuthStateChange may also update
      // userProfile asynchronously; we wait briefly before checking.
      // Redirect: if profile exists go home, otherwise go to setup.
      ...
      setError(msg);
      // BUG FIX: was `toast.error(error)` which reads the stale state value.
      toast.error(msg);
    } finally {
      setLoading(false);
    }
