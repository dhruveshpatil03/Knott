...
export default function PAGES_Dispute() {
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      setFormData(prev => ({
        ...prev,
        evidence: [...prev.evidence, ...newPhotos],
      }));
    } finally {
      setUploading(false);
    }
  ...
  const validateForm = () => {
    ...
    if (formData.evidence.length < 2) {
      newErrors.evidence = 'Minimum 2 photos required';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create dispute
      const dispute = await ordersService.createDispute({
        order_id,
        raised_by: user.id,
      ...
      <div className="max-w-2xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-3">
                Upload evidence (photos) * <span className="text-gray-600">(min 2, max 5)</span>
              </label>

              {/* Upload Area */}
              <label className="border-2 border-dashed border-gray-300 rounded-lg p-6 hover:border-red-400 transition cursor-pointer text-center">
                <input
                  type="file"
                  multiple
                  accept="image/*"
